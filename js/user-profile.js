...
function renderPopularityPreference(userProfileObj) {
     function formatData(obj){
          ...
          for(var i in obj){
               var temp;
               if (keys[i] % 5 == 0)
                    rtnObj.push({"popularity": keys[i], "value": obj[i]});
               console.log(keys[i] + ": " + obj[i]);
          }
	      return rtnObj;
     }
     ...
     const data = formatData(userProfileObj['songPopularity']); 
     ...
     var y = d3.scaleBand()
              .range([0, height])
              .domain(data.map(function(d) { return d.popularity }))
              .padding(.1);
     svg3.append("g")
          .call(d3.axisLeft(y))
          .style("stroke", "white")
...
fetch(userProfileUrl)
     ...
     .then(jsondata => {
          ...
          renderPopularityPreference(jsondata);
     });
